...
const REVERSE_FORMATS = Object.keys(FORMATS).reduce(function (obj, format) {
...
function parseCSVStream(s = new Set()) {
    ...
    return through(function (chunk) {
        ...
        let plen =
            ((isFirst && strarr.length - 1) || strarr.length) - parsed.length
...
function encodeValue(quote, v, _format, isArray) {
    ...
    switch (typeof v) {
        case 'string':
            if (isArray) {
                return `'${
                    ESCAPE_STRING[format] ? ESCAPE_STRING[format](v, quote) : v
                }'`
            }
...
class Rs extends Transform {
    ...
    writeRows(rows) {
        ...
        rows.forEach((row) => {
            writeString += ClickHouse.mapRowAsArray(row) + '\n'
        })

        let isOk = this.write(writeString)
    ...
    writeRow(data) {
        ...
        if (typeof data === 'string') {
            ...
            throw new Error(
                'Error: Inserted data must be an array, not an object.'
            )
        }

        let isOk = this.write(row + '\n')
        ...
        return new Promise((resolve, reject) => {
            me.ws
                ...
                .on('response', function (res) {
                    ...
                    res.on('data', (data) => (body += data)).on('end', () => {
                        res.body = body

                        return reject(getErrorObj(res))
                    })
                })
...
class QueryCursor {
    ...
    _getBodyForInsert() {
        ...
        if (Array.isArray(data) && data.every((d) => typeof d === 'string')) {
            ...
            throw new Error(
                'ClickHouse._getBodyForInsert: data is invalid format'
            )
        }
        ...
        if (isFirstElObject) {
            ...
            if (m) {
                ...
                throw new Error(
                    'insert query wasnt parsed field list after TABLE_NAME'
                )
            }
        }
    ...
    _getReqParams() {
        ...
        const { reqParams, config, username, password, database } =
            me.connection.opts
        ...
        if (typeof query === 'string') {
            ...
            if (query.match(/^(with|select|show|exists)/i)) {
                if (!R_FORMAT_PARSER.test(query)) {
                    query += ` FORMAT ${ClickHouse.getFullFormatName(
                        me.format
                    )}`
                }
                ...
                if (data && data.external) {
                    params['formData'] = data.external.reduce(function (
                        formData,
                        external
                    ) {
        ...
        if (
            me.opts.sessionId !== undefined &&
            typeof me.opts.sessionId === 'string'
        ) {
    ...
    exec(cb) {
        ...
        me._request = request.post(reqParams, (err, res) => {
            ...
            try {
                const data = this.opts.raw
                    ? res.body
                    : me.getBodyParser()(res.body)
    ...
    getBodyParser() {
        ...
        throw new Error(
            `CursorQuery.getBodyParser: unknown format "${this.format}"`
        )
    }
    ...
    getStreamParser() {
        ...
        throw new Error(
            `CursorQuery.getStreamParser: unknown format "${this.format}"`
        )
    }
    ...
    destroy() {
        ...
        throw new Error(
            'QueryCursor.destroy error: private field _request is invalid'
        )
    }
}
...
class ClickHouse {
    ...
    static getFullFormatName(format = '') {
        if (!FORMATS[format]) {
            throw new Error(
                `Clickhouse.getFullFormatName: unknown format "${format}`
            )
        }
    ...
    static getFormatFromQuery(query = '') {
        ...
        if (m) {
            ...
            if (!REVERSE_FORMATS[format]) {
                throw new Error(
                    `Clickhouse.getFormatFromQuery: unknown format "${format}"!`
                )
            }
    ...
    static getFormats() {
        return Object.keys(FORMATS).map((k) => ({
            format: k,
            fullFormatExpr: FORMATS[k],
        }))
    }
